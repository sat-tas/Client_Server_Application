    ...
    public class Parser
    {
        ...
        private void ParsArg(string input,double defaultValue,out double arg)
        {
            ...
            else
            {
                ...
                {
                    int funcid = funcs.GetValueOrDefault(groups["MathFunc"].Value);
                    double prewarg;
        ...
        private void FindOP(ref string input, char operation, List<string> funcArr, string replace)
        {
            ...
            while (input.IndexOf(operation) != -1)
            {
                ...
                int first = mid - 1;
                int last = mid + 1;
                while (first > 0 && !operatorsTemplates.Contains(input[first--])) ;
                while (last < input.Length && !operatorsTemplates.Contains(input[last++])) ;
                if (first != 0)
                    first += 2;
